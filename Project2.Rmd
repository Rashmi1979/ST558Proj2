...
bikeDataTrainF2 <- bikeDataTrain %>% select(cnt, atemp, yr, season,weathersit)
bikeDataTestF2 <- bikeDataTest %>% select(cnt, atemp, yr, season,weathersit)

bestLm <- lm(cnt ~ atemp + Year + season + weathersit, data = bikeDataTrain)
bestLm
summary(bestLm)
plot(bestLm)

bestLmFit <- train(cnt~., data= bikeDataTrainF2,
                   method = "lm",
                   preProcess = c("center", "scale"),
                   trControl = trainControl(method = "cv", number = 10))
lmFitPred <- predict(bestLmFit, newdata = dplyr::select(bikeDataTestF2,-cnt))

postResample(lmFitPred, bikeDataTestF2$cnt)
```
