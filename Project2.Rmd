...
BoostFitPred
postResample(BoostFitPred, bikeDataTest$cnt)

```

```{r}
finalModel <- as.formula("cnt ~ season + holiday + mnth + weathersit + atemp + windspeed + yr")

finalFit <- lm(finalModel, data = bikeDataTest)
coef(finalFit)

rmse <- sqrt(mean(finalFit$residuals^2))
rmse
